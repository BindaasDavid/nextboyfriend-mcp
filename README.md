...
Required env: `SOCAPI_KEY` or `SOCIAL_API_KEY` — **one** [SocialAPI.ai](https://docs.social-api.ai/) key for listing accounts, posting, and analytics (including **TikTok** once that account is connected in the SocialAPI dashboard). Article harvest for `fetch_articles` / automation reads **Supabase** (`articles`). Add `SUPABASE_ANON_KEY` (or service role) for CMS reads. Optional: `ANTHROPIC_API_KEY`, `HEYGEN_API_KEY` (see `.env.example`). In GitHub Actions, add the same SocialAPI key as secret `SOCAPI_KEY` and/or `SOCIAL_API_KEY`, plus Supabase secrets for automation.
